...
public class ExtendablePoolGameObject : ExtendablePool<PoolGameObject>
{
	...
	public ExtendablePoolGameObject(GameObject prefabGO, Transform parent) : base(null, parent)
	{
...
public class ExtendablePool<T> where T: MonoBehaviour, PoolableObject<T>
{
    ...
    public ExtendablePool(T prefab, Transform parent)
	{
		this.poolSize = 0;
		this.prefab = prefab;
		this.parent = parent;
    ...
    public virtual void SetInitialSize(int initialSize)
	{
        for(int i = poolSize; i < initialSize; i++)
		{
	...
	{
		if (!obj || !parent || HasAlreadyReturnedObject(obj))
			return;
